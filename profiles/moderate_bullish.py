"""
Moderate Bullish Profile - Balanced Risk/Reward Hedge Configuration

Moderate hedge ratios (0.40-0.90) with wider rebalancing and longer cooldowns.
Captures more upside while maintaining reasonable protection.
...
Best for: Users who like a bit of ETH/LINK upside, okay with some risk.
"""

# Profile metadata
PROFILE_NAME = 'moderate_bullish'
PROFILE_DESCRIPTION = 'Moderate bullish hedging with lower ratios for upside capture'

# ═══════════════════════════════════════════════════════════════════
# HEDGE RATIO CALCULATION
# ═══════════════════════════════════════════════════════════════════

# Correlation tiers - base hedge ratio (shifted -0.10 from balanced)
# Format: (min_correlation, hedge_ratio)
CORRELATION_TIERS = [
    (0.95, 0.88),  # Ultra-high correlation → 88% hedge (was 0.98)
    (0.88, 0.85),  # Very high correlation → 85% hedge (was 0.95)
    (0.78, 0.80),  # High correlation → 80% hedge (was 0.90)
    (0.68, 0.62),  # Medium correlation → 62% hedge (was 0.72)
    (0.55, 0.48),  # Low-medium correlation → 48% hedge (was 0.58)
    (0.00, 0.45),  # Low correlation → 45% hedge (raised from 0.40 - NEW FLOOR)
]

# Hedge ratio constraints
HEDGE_RATIO_FLOOR = 0.45  # Never go below 45% hedge (raised from 0.40)
HEDGE_RATIO_CAP = 0.90    # Cap at 90% (allow some upside)
...
MRHL_HEDGE_ADJUSTMENT = {
    ...
    'reduction': 0.15,  # Reduce by 15% (was 12% in balanced)
    'min_ratio': 0.45,  # Never go below 45% (raised from 0.40 - NEW FLOOR)
}
...
ARV_REBALANCING = {
    ...
    'high_arv': {
        'drift_multiplier': 1.4,    # More widening (was 1.3)
        'cooldown_multiplier': 1.6,  # More lengthening (was 1.5)
    },
    'low_arv': {
        'drift_multiplier': 0.90,   # Less tightening (was 0.85)
        'cooldown_multiplier': 0.80, # Less shortening (was 0.75)
    }
}
...
REBALANCING_REGIMES = {
    'aggressive': {  # Strong positive funding (≥ 0.12% daily)
        'delta_drift_threshold_pct': 0.38,  # Wider (was 0.30)
        'rebalance_cooldown_hours': 5.0,    # Longer (was 4.0)
        'funding_multiplier': 1.30,         # Weaker (was 1.50)
    },
    'mild_positive': {  # Mild positive funding (0.01% to 0.12% daily)
        'delta_drift_threshold_pct': 0.45,  # Wider (was 0.38)
        'rebalance_cooldown_hours': 10.0,   # Longer (was 8.0)
        'funding_multiplier': 1.05,         # Weaker (was 1.10)
    },
    'normal': {  # Neutral funding (-0.04% to 0.01% daily)
        'delta_drift_threshold_pct': 0.45,
        'rebalance_cooldown_hours': 10.0,
        'funding_multiplier': 1.00,
    },
    'lazy': {  # Negative funding (< -0.04% daily)
        # Use correlation-based formulas
        'use_correlation_formula': True,
        'drift_formula': lambda corr: max(0.90, 1.8 - 2.0 * corr) * 1.2,  # Wider
        'cooldown_formula': lambda corr: max(14.0, 32.0 - 32.0 * corr) * 1.2,  # Longer
        'funding_multiplier': 0.40,  # More reduction (was 0.50)
    }
}
...
MRHL_REBALANCING_OVERRIDES = {
    'ultra_fast': {  # MRHL ≤ 2.0h
        ...
        'comparison': '<=',
        'overrides': {
            'delta_drift_threshold_pct': 0.30,  # Wider (was 0.25)
            'rebalance_cooldown_hours': 2.5,    # Longer (was 2.0)
    ...
    'slow_trend': {  # MRHL ≥ 14.0h
        ...
        'comparison': '>=',
        'multipliers': {
            'delta_drift_threshold_pct': 2.5,  # More widening (was 2.2)
            'rebalance_cooldown_hours': 2.0,   # More lengthening (was 1.8)
        },
        'minimums': {
            'delta_drift_threshold_pct': 2.3,  # Higher floor (was 2.0)
            'rebalance_cooldown_hours': 30.0,  # Longer cap (was 24.0)
        },
        'overrides': {
            'down_threshold': -0.40,
...
ARV_DRIFT_CAP = {
    ...
    'max_drift': 0.75,  # Wider cap (was 0.60)
}
...
def calculate(corr_7d: float, arv_7d: float, mrhl_hours: float, funding_rate_daily: float) -> dict:
    """
    Calculate hedge configuration for moderate bullish profile.
    ...
    ratio = 0.45  # Default for very low correlation (raised from 0.40)
    ...
    if mrhl_hours > MRHL_HEDGE_ADJUSTMENT['threshold']:
        # Trending market - reduce hedge ratio by 15%
        ratio = max(
    ...
    # ═══════════════════════════════════════════════════════════════════
    # STEP 3: Final cap at 0.90 (allow some upside)
    # ═══════════════════════════════════════════════════════════════════
    
    ratio = min(0.90, ratio)
    ...
    # ═══════════════════════════════════════════════════════════════════
    # STEP 6.6: Apply hard cooldown cap (moderate bullish profile)
    # ═══════════════════════════════════════════════════════════════════
    
    # Hard cap at 30h for moderate bullish profile (longer than balanced's 24h)
    rebalance_cooldown_hours = min(rebalance_cooldown_hours, 30.0)
