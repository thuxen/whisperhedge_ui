"""
Moderate Bearish Profile - Balanced Risk/Protection Hedge Configuration

Higher hedge ratios (0.75-1.00) with tighter rebalancing and shorter cooldowns.
Provides more downside protection while maintaining some flexibility.
Uses 7-day log-return correlation as primary indicator for hedge ratio,
with moderate adjustments based on volatility (ARV), mean reversion (MRHL), and funding rates.

Best for: Users expecting bearish conditions, want downside protection without full lockdown.
"""

# Profile metadata
PROFILE_NAME = 'moderate_bearish'
PROFILE_DESCRIPTION = 'Moderate bearish hedging with higher ratios for downside protection'

# ═══════════════════════════════════════════════════════════════════
# HEDGE RATIO CALCULATION
# ═══════════════════════════════════════════════════════════════════

# Correlation tiers - positioned between balanced and full_protection
# Format: (min_correlation, hedge_ratio)
CORRELATION_TIERS = [
    (0.95, 0.99),  # Ultra-high correlation → 99% hedge (balanced: 0.98, full_protection: 1.00)
    (0.88, 0.97),  # Very high correlation → 97% hedge (balanced: 0.95, full_protection: 1.00)
    (0.78, 0.94),  # High correlation → 94% hedge (balanced: 0.90, full_protection: 1.00)
    (0.68, 0.87),  # Medium correlation → 87% hedge (balanced: 0.72, full_protection: 0.98)
    (0.55, 0.80),  # Low-medium correlation → 80% hedge (balanced: 0.58, full_protection: 0.97)
    (0.00, 0.75),  # Low correlation → 75% hedge (balanced: 0.45, full_protection: 0.95 - FLOOR)
]

# Hedge ratio constraints
HEDGE_RATIO_FLOOR = 0.75  # Never go below 75% hedge (sits between balanced 0.45 and full_protection 0.95)
...
MRHL_HEDGE_ADJUSTMENT = {
    ...
    'reduction': 0.08,  # Reduce by only 8% (less willing to reduce hedge than moderate_bullish)
    'min_ratio': 0.75,  # Never go below 75% (FLOOR)
}
...
ARV_REBALANCING = {
    ...
    'high_arv': {
        'drift_multiplier': 1.2,    # Less widening (was 1.4 in moderate_bullish)
        'cooldown_multiplier': 1.3,  # Less lengthening (was 1.6 in moderate_bullish)
    },
    'low_arv': {
        'drift_multiplier': 0.85,   # More tightening (was 0.90 in moderate_bullish)
        'cooldown_multiplier': 0.75, # More shortening (was 0.80 in moderate_bullish)
    }
}
...
REBALANCING_REGIMES = {
    'aggressive': {  # Strong positive funding (≥ 0.12% daily)
        'delta_drift_threshold_pct': 0.30,  # Tighter (was 0.38 in moderate_bullish)
        'rebalance_cooldown_hours': 4.0,    # Shorter (was 5.0 in moderate_bullish)
        'funding_multiplier': 1.50,         # Stronger (was 1.30 in moderate_bullish)
    },
    'mild_positive': {  # Mild positive funding (0.01% to 0.12% daily)
        'delta_drift_threshold_pct': 0.38,  # Tighter (was 0.45 in moderate_bullish)
        'rebalance_cooldown_hours': 8.0,    # Shorter (was 10.0 in moderate_bullish)
        'funding_multiplier': 1.10,         # Stronger (was 1.05 in moderate_bullish)
    },
    ...
    'lazy': {  # Negative funding (< -0.04% daily)
        ...
        'drift_formula': lambda corr: max(0.75, 1.8 - 2.0 * corr) * 1.0,  # Tighter (was 1.2 multiplier)
        'cooldown_formula': lambda corr: max(12.0, 32.0 - 32.0 * corr) * 1.0,  # Shorter (was 1.2 multiplier)
        'funding_multiplier': 0.50,  # Less reduction (was 0.40 in moderate_bullish)
    }
}
...
MRHL_REBALANCING_OVERRIDES = {
    'ultra_fast': {  # MRHL ≤ 2.0h
        ...
        'comparison': '<=',
        'overrides': {
            'delta_drift_threshold_pct': 0.25,  # Tighter (was 0.30 in moderate_bullish)
            'rebalance_cooldown_hours': 2.0,    # Shorter (was 2.5 in moderate_bullish)
            'down_threshold': -0.050,
    ...
    'slow_trend': {  # MRHL ≥ 14.0h
        ...
        'comparison': '>=',
        'multipliers': {
            'delta_drift_threshold_pct': 2.2,  # Less widening (was 2.5 in moderate_bullish)
            'rebalance_cooldown_hours': 1.8,   # Less lengthening (was 2.0 in moderate_bullish)
        },
        'minimums': {
            'delta_drift_threshold_pct': 2.0,  # Lower floor (was 2.3 in moderate_bullish)
            'rebalance_cooldown_hours': 24.0,  # Shorter cap (was 30.0 in moderate_bullish)
        },
        'overrides': {
            'down_threshold': -0.40,
            'bounce_threshold': -0.30,
            'lookback_hours': 60,
        }
    }
}
...
SAFETY_FORMULAS = {
    'max_hedge_drift_pct': lambda corr: max(0.40, min(0.60, 0.50 + 1.0 * (0.75 - corr))),  # Tighter range
    'drift_min_pct_of_capital': lambda corr: 0.08 * (corr / 0.75),  # Slightly higher sensitivity
}

# ARV-based drift cap (low volatility override)
ARV_DRIFT_CAP = {
    'threshold': 25.0,  # ARV <= 25% = very low volatility
    'max_drift': 0.60,  # Tighter cap (was 0.75 in moderate_bullish)
}
...
def calculate(corr_7d: float, arv_7d: float, mrhl_hours: float, funding_rate_daily: float) -> dict:
    """
    Calculate hedge configuration for moderate bearish profile.
    ...
    ratio = 0.75  # Default for very low correlation (FLOOR)
    ...
    if mrhl_hours > MRHL_HEDGE_ADJUSTMENT['threshold']:
        # Trending market - reduce hedge ratio by only 8% (less willing than moderate_bullish)
        ratio = max(
    ...
    if arv_7d >= ARV_REBALANCING['high_threshold']:
        # High volatility - less widening than moderate_bullish
        ...
        # Low volatility - more tightening than moderate_bullish
    ...
    if arv_7d <= ARV_DRIFT_CAP['threshold']:
        # Very low volatility - cap drift threshold (tighter than moderate_bullish)
    ...
    if corr_7d < 0.55:
        delta_drift_threshold_pct = min(delta_drift_threshold_pct, 1.50)  # Tighter than moderate_bullish
    
    # ═══════════════════════════════════════════════════════════════════
    # STEP 6.6: Apply hard cooldown cap (moderate bearish profile)
    # ═══════════════════════════════════════════════════════════════════
    
    # Hard cap at 24h for moderate bearish profile (shorter than moderate_bullish's 30h)
